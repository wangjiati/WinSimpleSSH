...
| 字段 | 值 |
|------|-----|
| 文件 | `src/SSHClient/Core/RemoteShell.cs:30`、`src/SSHServer/WebSocketServerEngine.cs:25` |
| 类别 | 加密 |
| 状态 | 待修复（计划采用查表+偏移量报文混淆方案） |

```csharp
var url = $"ws://{host}:{port}/";
```

所有认证凭据、Shell 命令、文件内容以明文传输。安全团队随机抓包可直接看到原文。
虽有 IP 白名单限制连接来源，但不防同网段嗅探。

**计划方案：** 不走 TLS 证书，采用查表替换 + 滚动偏移的报文混淆，每连接随机种子派生
查找表，让抓包工具无法直接读取原文。速度远快于 AES。
