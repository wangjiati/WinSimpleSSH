...
**服务端帮助命令：**
- `SSHServer.exe` 新增 `help`、`-h`、`--help`、`-?`、`/?` 等帮助参数，输出中英双语用法后立即退出，不启动 WebSocket 服务
- 帮助参数优先于其他启动参数，例如 `SSHServer.exe --console help` 只显示帮助
- 兼容 WinExe 在不同启动上下文中的输出：cmd / Git Bash / 重定向直接输出，PowerShell 附着父控制台，双击运行时创建控制台并等待按键
- 管道 / 重定向输出使用无 BOM UTF-8，真实控制台沿用当前代码页，既便于脚本解析，也不会污染父 cmd / PowerShell 的编码状态
- 默认静默后台模式继续保持 stdout 无输出；`--console` 启动横幅与帮助信息统一为中英双语

**测试与升级说明：**
- 新增 `tests/16-server-help.bat`，覆盖 9 种帮助参数、退出码、Usage 输出及无新增残留进程，并保留测试前已经运行的服务端实例
- 本版本仅涉及服务端功能，现场仅需替换 `SSHServer.exe`；`SSHC.exe` 无功能变化
...
**批处理中文乱码修复：**
- 修复 `.bat` 批处理连续调用 `SSHC` 时中文参数乱码的问题：非交互模式设置 UTF-8 输出（`SetConsoleOutputCP(65001)`）会改动与父 cmd.exe 共享的控制台代码页，且在进程退出后持续生效；父 cmd 按读取每一行时的代码页解码批处理，导致后续行中文参数按错误编码解码
- 新增 `ConsoleEncodingScope`：进入时保存原代码页，在所有退出路径（正常返回、异常、Ctrl+C 看门狗 `Environment.Exit`）恢复原值，不再污染父进程控制台状态
- 运行期间 stdout 仍强制 UTF-8，Agent / 脚本调用行为不变；交互模式不受影响
- 新增回归测试 `tests/15-encoding-restore.bat`（无需服务端即可验证代码页恢复）

**批处理编码使用建议：**
- bat 保存为 ANSI/GBK（记事本默认）可直接使用
- bat 保存为 UTF-8 时需在首行加 `chcp 65001 >nul`，两种方式均支持
...
**大文件传输可靠性修复：**
- 上传和下载改为流式传输，避免大文件一次性读入内存
- 上传增加分块顺序、实际字节数、分块数量和 SHA-256 完整性校验
- 下载增加分块顺序、实际字节数、分块数量和 SHA-256 完整性校验
- 上传/下载校验失败时，非交互 CLI 会正确返回失败状态
- 修复上传等待 `UploadReady` 的竞态问题，服务端拒绝传输时不再长时间等待

**验证：**
- 使用约 202MB 的 `prometheus.exe` 和约 180MB 的 `promtool.exe` 完成上传/下载测试
- 上传后远端 SHA-256 与本地一致，下载后本地 SHA-256 与远端一致
